...
export async function getCalendarEvents(organizationId: string, start: Date, end: Date, userId?: string) {
  const conditions = [
...
export async function getEventById(organizationId: string, id: string) {
  const result = await db
    .select()
    .from(calendarEvents)
    .where(and(eq(calendarEvents.organizationId, organizationId), eq(calendarEvents.id, id)))
    .limit(1);
  return result[0] ?? null;
}

export async function getDeadlines(organizationId: string, filters: { caseId?: string; userId?: string; overdue?: boolean } = {}) {
...
export async function getTasks(organizationId: string, filters: { caseId?: string; userId?: string; status?: string } = {}) {
...
export async function getBringUps(organizationId: string, filters: { caseId?: string; status?: string } = {}) {
  const conditions = [eq(bringUps.organizationId, organizationId)];
